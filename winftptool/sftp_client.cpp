	...
	const char* passwd, const char* host, int port, char* sftppath)
{
	...
	printf("user=%s,passwd=%s,host=%s,port=%d\r\n",user,passwd,host,port);
	uint32_t hostaddr;
	int i, ret;
	int auth_pw = 0;
	...
	tsc_sftp_handle_t* sftp_handle  = new tsc_sftp_handle_t;//malloc(sizeof(tsc_sftp_handle_t));
	if(sftp_handle ==NULL)
		return NULL;
	
	hostaddr = inet_addr("192.169.77.22");

	//初始化ssh2库
	ret = libssh2_init (0);
	if (ret != 0) 
	{
		WriteLog("SFTP : libssh2 initialization failed (%d)\n", ret);
		return NULL;
	}
	...
	{
		WriteLog("SFTP : failed to create session!\n");
		return NULL;
	}
	...
	if(ret) 
	{
		WriteLog("SFTP : Failure establishing SSH session: %d\n", ret);
		return NULL;
	}
	...
	if (auth_pw & 1) 
	{
		printf("SFTP : Authentication by password.\n");
		if (libssh2_userauth_password(sftp_handle->ssh2_session, user, passwd)) 
		{
			printf("SFTP : Authentication by password failed.\n");
		...
		{
			printf("SFTP : Authentication by password success.\n");
		}
	...
	{
		WriteLog("SFTP : No supported authentication methods found!\n");
		libssh2_session_disconnect(sftp_handle->ssh2_session, "SFTP auth fail.");
		libssh2_session_free(sftp_handle->ssh2_session);
		close(sftp_handle->socket);
		libssh2_exit();
	...
	if(sftppath != NULL)
	{
	    sftp_handle =(tsc_sftp_handle_t*)libssh2_sftp_open(sftp_handle->sftp_session, sftppath, LIBSSH2_FXF_READ, 0);

	    if(sftp_handle == NULL) 
		{
	        printf("SFTP : Unable to open file with SFTP: %ld\n",
...
int tsc_sftp_readdir(void* handle, const char* path,tsc_sftp_dir_info_t* dir_info)
{
	...
	if (!_handle)
	{
		WriteLog("Unable to open dir with SFTP\n");
		return -1;
    }	
